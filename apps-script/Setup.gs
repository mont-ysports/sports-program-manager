...
function _setupRegistrationsSheet(ss) {
  ...
  try {
    ...
    sheet.getRange('A2:AA1000').applyRowBanding(
      SpreadsheetApp.BandingTheme.LIGHT_GREY, false, false
    );
  } catch (e) { /* ignore if banding not supported */ }
...
function _setupConfigSheet(ss) {
  ...
  var defaults = [
    ['CHECKIN_PIN',           '1234',                    'PIN required by staff to use the check-in station'],
    ['PAYMENT_AMOUNT',        '75',                      'Registration fee in USD'],
    ['PAYMENT_INSTRUCTIONS',  'Make payment via bank transfer to Account: 1234567890. Use your Registration ID as reference.', 'Payment instructions shown to parents'],
    ['APP_URL',               'https://your-app.onrender.com', 'Deployed frontend URL (used in emails)'],
